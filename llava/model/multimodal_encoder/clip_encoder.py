    ...
    def forward(self, images):
        if type(images) is list:
            ...
            for image in images:
                ...
                image_feature = self.token_prune_merge_advanced_plus(image_feature, if_adaptive=True, reduction_ratio=1/8)
                image_features.append(image_feature)
        else:
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True)
            image_features = self.feature_select(image_forward_outs).to(images.dtype)

            # Choose one of the prune merge methods to apply
            image_features = self.token_prune_merge_advanced_plus(image_features, if_adaptive=True, reduction_ratio=1/8)
